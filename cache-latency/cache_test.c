    ...
    for (i = 0; i < 32; i++) {
        ...
        asm("cpuid"      // call i386 cpuid instruction
            : "+a"(eax)  // contains the cpuid command code, 4 for cache query
              ...
              ,
              "=d"(edx));  // generates output in 4 registers eax, ebx, ecx and
                           // edx

        // taken from
        // http://download.intel.com/products/processor/manual/325462.pdf Vol.
        // 2A 3-149
        ...
        // taken from
        // http://download.intel.com/products/processor/manual/325462.pdf 3-166
        // Vol. 2A ebx contains 3 integers of 10, 10 and 12 bits respectively
...
int test_cache(size_t attempts, size_t lower_cache_size, int* latencies,
    ...
    char* random_data = mmap(NULL, lower_cache_size, PROT_READ | PROT_WRITE,
...
int main() {
    ...
    for (i = 0; i < num_data_caches; i++) {
        test_cache(attempts, cache_sizes[i] * 4, latencies,
                   sizeof(latencies) / sizeof(*latencies));
