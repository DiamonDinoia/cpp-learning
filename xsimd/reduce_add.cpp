...
constexpr auto shsum(const auto& v, auto& t, std::integer_sequence<int, i...>) noexcept {
    const auto low_high = xsimd::swizzle(v, deinterleave_mask<typename std::decay_t<decltype(v)>::arch_type, float>);
    t[0] = (low_high.data[i] + ...);
...
int main(const int argc, const char* argv[]) {
    ...
    alignas(batch_type::arch_type::alignment()) std::array<float, 2> out{};
    ...
    ankerl::nanobench::Bench().run("reduce_add", [&] constexpr noexcept {
        const auto res_real = xsimd::shuffle(a, b, select_even_mask<typename batch_type::arch_type, float>);
        const auto res_imag = xsimd::shuffle(a, b, select_odd_mask<typename batch_type::arch_type, float>);
